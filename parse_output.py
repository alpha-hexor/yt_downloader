...
def parse(result):
    ...
    x=r.split("\n")[2:]
    p=[ re.findall(r'(\d+|\w+)\s+(\w+)\s+(audio_only|\d+x\d+)\s+(\d+|)\s+(\d+|)\s+\|\s+(\d+.\d+MiB|KiB|GiB|).*\|\s+(audio_only|\w+|\w+\.\w+|\w+\.\w+\.\w+|\w+\.\w+\.\w+\.\w+)\s+(\d+k|)\s+(\w+|video_only|\w+\.\w+|\w+\.\w+\.\w+|\w+\.\w+\.\w+\.\w+).*',i) for i in x]
    p=p[:-1]
